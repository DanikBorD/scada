...
namespace mcOMRON
{
	...
	public class tcpFINSCommand : IFINSCommand
	{
		...
		/// <returns></returns>
		public bool MemoryAreaRead(MemoryArea area, UInt16 address, UInt16 count)
		{
			try
			{
				// command & subcomand
		...
		private bool FrameSend(Byte[] data)
		{
			...
			{
				this._lastError += string.Format("Response Code error: (Codi: {0}  Subcodi: {1})",
